...
try:
    ...
    print("警告: pyrealsense2 未安装，RealSense功能将不可用")
...
try:
    ...
    SAVE_WIDTH = 1280
    SAVE_HEIGHT = 720
    FPS = 15  # 改为30FPS匹配acamera的配置
    CONNECTION_TIMEOUT = 10
    BUFFER_SIZE = 1
    OUTPUT_DIR = "ip_camera_videos"
    TIMESTAMP_FORMAT = "%Y%m%d_%H%M%S"

# 视频录制参数
OUTPUT_FORMAT = "avi"         # 输出格式：avi, mp4
CODEC = "XVID"                # 编码器：XVID, MJPG, mp4v
VIDEO_OUTPUT_DIR = "ip_camera_videos"  # 视频输出目录

# RealSense 相机配置（可选，从配置文件读取，如果没有则使用默认值）
# 如果camera_config.py中定义了这些变量，会被自动导入；否则使用下面的默认值
try:
    # 检查是否从配置文件导入
    _ = USE_REALSENSE
    print(f"从配置文件导入 USE_REALSENSE = {USE_REALSENSE}")
except NameError:
    USE_REALSENSE = False
    print("配置文件中未定义 USE_REALSENSE，使用默认值 False")

print(f"最终设置 USE_REALSENSE = {USE_REALSENSE}")
...
DEPTH_COLORMAP = cv2.COLORMAP_JET  # 深度图可视化颜色映射
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                self.connection_lost = False
                self.retry_count = 0
                print(f"  {self.ip}: Video stream connected")
...
def test_camera_connection(ip):
    ...
    print(f"  Testing camera {ip}: {url.replace(CAMERA_PASSWORD, '***') if CAMERA_PASSWORD else url}")
...
def setup_realsense():
    ...
    try:
        ...
        # 使用与acamera完全相同的配置
        # print("  Configuring streams: COLOR=1280x720, DEPTH=1280x720, FPS=30")
...
def main():
    ...
    ip_list = [f"{IP_CAMERA_BASE}{i}" for i in range(IP_START, IP_END + 1, 2)]  # 隔一个使用
    # ip_list = ['192.168.1.14']  # 先测试单台相机是否工作
    print(f"Testing IP list: {ip_list}")
    
    print(f"\nMulti-threaded IP Camera Video Recording Configuration:")
    print(f"  Camera Type: {CAMERA_TYPE}")
    print(f"  IP Range: {ip_list[0]} - {ip_list[-1] if len(ip_list) > 1 else ip_list[0]} ({len(ip_list)} cameras)")
    print(f"  Username: {CAMERA_USERNAME}")
    print(f"  Resolution: {SAVE_WIDTH}x{SAVE_HEIGHT} @ {FPS} FPS")
    ...
    print(f"USE_REALSENSE = {USE_REALSENSE}, REALSENSE_AVAILABLE = {REALSENSE_AVAILABLE}")
    if USE_REALSENSE:
        print("\nInitializing RealSense camera...")
        print(f"RealSense config: COLOR={COLOR_WIDTH}x{COLOR_HEIGHT}, DEPTH={DEPTH_WIDTH}x{DEPTH_HEIGHT}, FPS=30 (hardcoded)")
    ...
    print(f"\n{'='*60}")
    print(f"Multi-threaded Preview Mode ({total_cameras} cameras)")
    ...
    try:
        while True:
            ...
            if recording_started and start_time is None:
                ...
                for thread in active_threads:
                    ...
                    output_filename = os.path.join(
                        VIDEO_OUTPUT_DIR,
                        f"ipcam_{thread.ip}_{timestamp}.{OUTPUT_FORMAT}"
                    )
                    video_writers[thread.ip] = cv2.VideoWriter(
                        output_filename,
                        fourcc,
                        FPS,
                        (SAVE_WIDTH, SAVE_HEIGHT)
                    )
                    print(f"Recording IP cam {thread.ip} to: {output_filename}")
                
                # 创建 RealSense 的视频写入器
                if realsense_pipeline:
                    color_filename = os.path.join(VIDEO_OUTPUT_DIR, f"realsense_color_{timestamp}.{OUTPUT_FORMAT}")
                    ...
                    if SAVE_DEPTH_AS_PNG:
                        realsense_depth_dir = os.path.join(VIDEO_OUTPUT_DIR, f"realsense_depth_{timestamp}")
                        os.makedirs(realsense_depth_dir, exist_ok=True)
                        print(f"Recording RealSense depth (16-bit PNG) to: {realsense_depth_dir}/")
                    else:
                        depth_filename = os.path.join(VIDEO_OUTPUT_DIR, f"realsense_depth_{timestamp}.{OUTPUT_FORMAT}")
            ...
            if recording_started and start_time is not None:
                ...
                for ip, frame in frames.items():
                    ...
                    if save_frame is not None and ip in video_writers:
                        video_writers[ip].write(save_frame)
            ...
            for ip, frame in frames.items():
                ...
                if display_frame is not None:
                    ...
                    if recording_started and start_time is not None:
                        ...
                        info_text = f"{ip} | REC | Frame: {frame_count} | Time: {duration:.1f}s"
                        info_text2 = f"FPS: {fps:.1f} | Sync: {sync_count}/{len(active_threads)}"
                        ...
                        # 预览模式
                        sync_count = len(frames)
                        info_text = f"{ip} | PREVIEW - Press SPACE to START"
                        info_text2 = f"FPS: {fps:.1f} | Sync: {sync_count}/{len(active_threads)}"
                        color = (0, 255, 255)  # 黄色表示预览
                    
                    cv2.putText(display_frame, info_text, (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
                    cv2.putText(display_frame, info_text2, (10, 50), 
        ...
        for ip, writer in video_writers.items():
            try:
                writer.release()
                print(f"  Saved video for IP camera {ip}")
            except:
                pass
        ...
        if start_time is not None:
            ...
            print(f"Videos saved in: {os.path.abspath(VIDEO_OUTPUT_DIR)}")
            ...
            print(f"\n{'='*60}")
            print("No recording was made (preview mode only)")
            print(f"{'='*60}")

if __name__ == "__main__":
    print("IP Camera Video Recording System")
    print("=" * 40)
    main()
